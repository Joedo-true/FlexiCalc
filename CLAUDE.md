...
A multi-step calculator that quotes renovation work and collects the lead at the
end. The whole app is one hand-written `index.html` — markup, styles and vanilla
JS in a single file. No build step, no package manager, no dependencies, no
framework: open the file in a browser and it runs.
`.github/workflows/deploy-pages.yml` publishes the repository root to GitHub
Pages as-is, and `.nojekyll` keeps Jekyll from touching it.

Edit `index.html` directly — there is nothing to compile and nothing to rebuild.
Keep it readable: the `<style>` block is split into labelled sections, and new
logic belongs in the existing `<script>` rather than in a second one.
